...
# The API key this server instance authenticates as -- issued immediately at
# checkout when subscribing at https://goodsong.dev/#pricing (no free tier).
# Set per MCP client config, e.g. `"env": {"VERIFY_API_KEY": "vk_..."}` in the
# client's mcp.json.
